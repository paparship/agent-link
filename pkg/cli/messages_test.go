...
func setupAgentEnv(t *testing.T, serverURL string) string {
	...
	writeConfigTOML(filepath.Join(agentlinkDir, "config.toml"), serverURL, "test-device", homeDir)
...
func TestRunSend(t *testing.T) {
	...
	t.Run("send with short name", func(t *testing.T) {
		...
		if err := RunSend("worker", "hello"); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("send with full name", func(t *testing.T) {
		...
		if err := RunSend("other-dev:reviewer", "hi there"); err != nil {
			t.Fatal(err)
		}
	...
	t.Run("send with multi-line content", func(t *testing.T) {
		...
		content := "line one\nline two\nline three"
		if err := RunSend("worker", content); err != nil {
			t.Fatal(err)
		}
...
func TestLoadConfig(t *testing.T) {
	...
	os.MkdirAll(filepath.Join(homeDir, ".agentlink"), 0755)
	writeConfigTOML(filepath.Join(homeDir, ".agentlink", "config.toml"), "http://srv:8080", "test-dev", "/tmp")
