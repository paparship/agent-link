...
创建 `main/` 和 `worker/` 目录，各含 `.agentlink.toml` + `CLAUDE.md`。自动启动两个 tmux session（`main`、`worker`）运行 Claude Code，每个 session 附带一个后台轮询进程。
...
- **客户端**: 通过 HTTP API 收发消息，通过 tmux 与 Claude 交互
- **轮询器**: 后台检测收件箱，Claude 空闲时自动注入消息
